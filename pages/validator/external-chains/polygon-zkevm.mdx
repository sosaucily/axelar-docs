...
### Prerequisites
This is the most straightforward path to run a zkEVM node, and it's perfectly fine forfor instance it's not needed to use Docker, you could use the Go and C++ binaries directly.
Some instructions to install Prerequisites are give here however be sure to check the [official documentation](https://docs.do most use case.
This is quite opinionated, feel free to run this software in a different way, cker.com/compose/install/) to install docker compose in case this doesn't work.
```bash
...
# zkEVM NODE POOLDB DATA DIR
mkdir -p ~/.zkevm/data/pooldb
```

### Setup configs

Setting an env var to run subsequent cmds more easily
<Tabs tabs={[
    {
        title: "Mainnet",
...
Update the env vars in `$ZKEVM_CONFIG_DIR/.env`
```bash
# Either set to mainnet or testnet
ZKEVM_NETWORK=mainnet/testnet
# Set this to your Ethereum node RPC
ZKEVM_NODE_ETHERMAN_URL="your Ethereum node's RPC"
...
_Optional:_ We will need to use directories defined earlier when we want to pass config and env file to docker-compose, it would be nice to persist them across sessions
Add them to your shell file, for example `~/.bashrc` or `~/.zshrc`.
```bash
ZKEVM_NET=testnet
ZKEVM_DIR=~/.zkevm
ZKEVM_CONFIG_DIR=~/.zkevm/config
...
You can compare the block height on your node with the explorer ([mainnet](https://zkevm.polygonscan.com) or [testnet](https://testnet-zkevm.polygonscan.com)),
use your RPC node only when it has caught up with the latest block height.

### Configure vald

In order for `vald` to connect to your Polygon zkEVM node, your `rpc_addr` should be exposed in
vald's `config.toml`
...
To update the zkEVM node, repeat the setup steps, being careful not to overwrite the configuration files that you have modified.
